...
Do
    ...
    If PROD_MOD <> "00" Then
        Call commandScreen(whichKey:="<clear>")
        Call commandScreen(screenType:="pibc", symbol:=symbol, number:=number, module:=Format(PROD_MOD, "00"), whichKey:="<Enter>")
        policyArchived = screen.getstring(1, 54, 17)
            If policyArchived = "POLICY ON HISTORY" Then
                ActiveCell.Offset(0, 1).Select
                ActiveCell = "Archived"
                MOD_COUNT = 4
            End If
    End If
Loop Until (PROD_MOD <= -1) Or MOD_COUNT >= 3
